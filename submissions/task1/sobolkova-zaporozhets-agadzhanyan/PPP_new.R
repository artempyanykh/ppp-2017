...
#-ITEM 3 --------------------------------------------------------------------------------------
#ppptesting.01 <- read.csv("/Users/Anastasia/Desktop/testing.csv")
ppptesting.01  read.table("/Users/Anastasia/Desktop/testing.csv", header = T)
ppptesting.01$Date = as.Date(ppptesting.01$Date)
...
prognoz1 <- forecast(mod_1, h = length(ppptesting.01$Date))
plot(prognoz)
AIC(mod_1)

mod_2 <- Arima(pppdata.02, order = c(2,1,0))
prognoz2 <- forecast(mod_2, h = length(ppptesting.01$Date))
plot(prognoz2)
AIC(mod_2)

mod_3 <- Arima(pppdata.02, order = c(0,1,4))
prognoz3 <- forecast(mod_3, h = length(ppptesting.01$Date))
plot(prognoz3)
AIC(mod_3)

mod_4 <- Arima(pppdata.02, order = c(2,1,4))
prognoz4 <- forecast(mod_4, h = length(ppptesting.01$Date))
plot(prognoz4)
AIC(mod_4)
...
prognoza <- forecast(mod_a, h = length(ppptesting.01$Date))
plot(prognoza)
AIC(mod_a)

R2_mod_1<-R2_Score(prognoz1$mean, ppptesting.01$Value)
R2_mod_2<-R2_Score(prognoz2$mean, ppptesting.01$Value)
R2_mod_3<-R2_Score(prognoz3$mean, ppptesting.01$Value)
R2_mod_4<-R2_Score(prognoz4$mean, ppptesting.01$Value)
stick<-rbind(pppdata.01,ppptesting.01)
stick

#plot(ppptesting.01, style = "l")
plot(prognoz4) 
lines(stick$Value, type = "l", col = "red")

ppptesting.02 <- ts(data = pppdata.01[ ,2], frequency = 12, start = c(1989,01,01))
plot(ppptesting.02, type="l", col=2)

plot(prognoz4) & lines(ppptesting.01, type="l", col=2)
